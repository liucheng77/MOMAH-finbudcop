/* =========================================================================
   ...
   -------------------------------------------------------------------------
   Verbatim-mirrored from src/App.jsx so the G-04 department pages can be
   maintained / handed off independently.  src/App.jsx remains the LIVE source
   of truth; this module is a self-contained copy (intentional duplication).
   Runtime deps: react, react-dom only.
   A local store shim (G04StoreProvider / useStore) lets the G04 pages render in
   isolation; when embedded back in the real app, swap in the host StoreProvider.
   ========================================================================= */
import React, { createContext, useContext, useState, useEffect, useRef } from "react";
import { createPortal } from "react-dom";

const SHOW_UC = true;            // show UC-xx codes (mirrors App.jsx demo mode)
const PLAZA_G06 = null;          // fallback only — G04 always passes PLAZA_G04
const RC_FLOW = [];              // fallback only — G04 cfgs always define `flow`

/* ---- helpers (verbatim from src/App.jsx) ---- */
...
/* ---- generic UI components (verbatim from src/App.jsx) ---- */
...
/* =========================================================================
   Local store shim — lets G04 pages render standalone (isolated preview /
   handoff). Mirrors the slice of App.jsx's StoreProvider that the G04
   components consume (tr / t / lang / setRoute / pushLog / ...).
   ========================================================================= */
const G04Store = createContext(null);
function G04StoreProvider({ children, initial = {} }) {
  const [lang, setLang] = useState(initial.lang || "en");
  const [route, setRoute] = useState(initial.route || "entwork");
  const [deptSub, setDeptSub] = useState(initial.deptSub || null);
  const dir = lang === "ar" ? "rtl" : "ltr";
  const tr = (o) => (o && typeof o === "object" ? (o[lang] != null ? o[lang] : (o.en != null ? o.en : o.ar)) : o);
  const t = (k) => k;
  const pushLog = () => {};
  const value = {
    lang, setLang, dir, route, setRoute, deptSub, setDeptSub, t, tr, clean: (s) => s,
    cycleLang: () => setLang((l) => (l === "ar" ? "en" : l === "en" ? "zh" : "ar")),
    user: null, setUser: () => {}, cov: "consolidated", setCov: () => {},
    alerts: [], ackAlert: () => {}, log: [], reports: [], addReport: () => {},
    pendingQ: null, setPendingQ: () => {}, perfJump: null, setPerfJump: () => {},
    backRoute: null, setBackRoute: () => {}, alertsOpen: false, setAlertsOpen: () => {},
    curMode: "riyal", setCurMode: () => {}, askOrchestrator: () => {}, reset: () => {},
  };
  return React.createElement(G04Store.Provider, { value }, children);
}
function useStore() { return useContext(G04Store); }

export {
  SHOW_UC, sanitizeUc, ucl, Money, F6_AG, BQ_POS, Hi, OrchNext,
  askHiAgent, mdToJsx,
  DirectorateFlow, KpiCarousel, BusinessPlaza, WsFlowCard, OrchChat, WsConsoleCard, DeptWorkspace, UcBench,
  G04StoreProvider, useStore,
};

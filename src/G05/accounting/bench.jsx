/* 会计部 — UC-09 财务关账、对账与结算 (Accounting Department 专属工作台).
   完整复制自财务权益部(G-04)的 UC-09 工作台(G04/entitlements/bench09.jsx),
   改为会计部(G-05)语境:
   - route 改为 "g05bench09",back 改为 "acctwork"(返回会计部广场)
   - deptName 改为会计部,dept 改为 "acct"
   - chain/chainLab 改为 G-05 会计部链路(UC-08 → UC-09 → UC-11 → UC-10 → UC-02/03)
   - ctas 的 UC-10 去向改为 "reports"(frep),贴合 G-05 语境
   - summary/recs/scope/outputs/sources/roles/logs/narr/qs/answers/genAns 等业务数据
     与财务权益部版本保持一致(两部门 UC-09 业务内容相同)。
   渲染复用 ./G04 的富工作台组件 UcBenchG04(按 cfg.uc === "UC-09" 展示
   结算/对账/清算明细表),由 App.jsx 的 g05bench09 路由挂载。 */
const BENCH_UC09_G05 = {
  route: "g05bench09", back: "acctwork", dept: "acct", tone: "violet", uc: "UC-09", run: "#9027", tool: "uc09", chainHideUc: true,
